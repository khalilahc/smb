...
const RemoteGridCard: React.FC<GridCardProps> = ({ peerId }) => {
  ...
  return (
    <div className="relative bg-white border-2 border-pink-300 rounded-xl p-2 shadow-md transition transform hover:scale-105 w-full max-w-xs">
      {/* Video */}
      ...
      <div className="text-center mt-2">
        ...
        />
        <h4 className="font-bold text-purple-700 mt-1">
          {metadata?.displayName || "Unnamed Queen"}
        </h4>
        <p className="text-sm text-gray-500 capitalize">{role || "Participant"}</p>
        {metadata?.isHandRaised && (
          <span className="block mt-1 text-yellow-500 animate-bounce">
            ✋ Hand Raised
          </span>
        )}
      </div>
